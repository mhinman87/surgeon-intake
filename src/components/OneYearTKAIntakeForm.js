...
import OneYearTKAComplaintForm from './OneYearTKAComplaintForm';
...
export default function OneYearTKAIntakeForm() {
  ...
  const getFieldsForStep = (step) => {
    switch (step) {
      case 0:
        return [
          'kneeSide', 'surgeryType', 'surgeryDate', 'surgeryLocation', 'surgeon',
  ...
  const onSubmit = (data) => {
    ...
    try {
      ...
      generateOneYearTKAReportPDF(data);
  ...
  const renderStepContent = (step) => {
    switch (step) {
      case 0:
        return <OneYearTKAComplaintForm />;
      case 1:
        return <MedicalHistoryPrompt />;
      case 2:
        return <OneYearTKAReviewForm />;
      default:
        return 'Unknown step';
    }
  };
      ...
      <Box sx={{ width: '100%' }}>
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            1-year TKA intake form submitted successfully! PDF report generated.
          </Alert>
        </Snackbar>
      </Box>
    </FormProvider>
