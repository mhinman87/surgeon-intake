...
import TKAComplaintForm from './TKAComplaintForm';
import MedicalHistoryForm from './MedicalHistoryForm';
import TKAReviewForm from './TKAReviewForm';
import { generateTKAReportPDF } from '../utils/tkaPdfGenerator';

const steps = ['TKA Evaluation', 'Medical History', 'Review & Submit'];

const schema = yup.object({
  // TKA Evaluation fields
  ...
  injurySinceTKA: yup.string().required('Please select injury history since TKA').notOneOf([''], 'Please select injury history since TKA'),
  injuryDescription: yup.string().when('injurySinceTKA', {
    is: 'positive',
  ...
  // Medical History fields (same as other forms)
...
export default function TKAIntakeForm() {
  ...
  const onSubmit = (data) => {
    console.log('TKA form submitted:', data);
    console.log('About to generate PDF...');
    
    // Also show an alert for immediate feedback
    alert('TKA evaluation form submitted! PDF is being generated. Check your downloads folder or look for a new tab/window.');
    
    try {
      // Generate and open PDF
      generateTKAReportPDF(data);
  ...
  const getFieldsForStep = (step) => {
    switch (step) {
      case 0:
        return [
          'kneeSide', 'originalSurgeon', 'originalLocation', 'originalYear', 'complicationsHistory',
  ...
  const renderStepContent = (step) => {
    switch (step) {
      case 0:
        return <TKAComplaintForm />;
      case 1:
        return <MedicalHistoryForm />;
      case 2:
        return <TKAReviewForm />;
      default:
        return null;
    }
  };
      ...
      <Box>
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            TKA evaluation form submitted successfully! PDF report generated.
          </Alert>
        </Snackbar>
      </Box>
    </FormProvider>
