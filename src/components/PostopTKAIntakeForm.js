...
import PostopTKAComplaintForm from './PostopTKAComplaintForm';
import MedicalHistoryPrompt from './MedicalHistoryPrompt';
import MedicalHistoryForm from './MedicalHistoryForm';
import PostopTKAReviewForm from './PostopTKAReviewForm';
import { generatePostopTKAReportPDF } from '../utils/postopTkaPdfGenerator';
import { generatePatientReportText, copyToClipboard } from '../utils/textGenerator';

const steps = ['Post-operative Information', 'Medical History', 'Review & Submit'];
...
const schema = yup.object().shape({
  ...
  symptomRelief: yup.string().required('Symptom relief level is required'),
  satisfaction: yup.string().required('Satisfaction status is required'),
  ...
  kneeExtension: yup.string().required('Knee extension ROM is required'),
  kneeFlexion: yup.string().required('Knee flexion ROM is required'),
...
export default function PostopTKAIntakeForm() {
  ...
  const getFieldsForStep = (step) => {
    switch (step) {
      case 0:
        return [
          'kneeSide', 'surgeryType', 'surgeryDate', 'surgeryLocation', 'historyChanges',
  ...
  const onSubmit = (data) => {
    ...
    try {
      ...
      generatePostopTKAReportPDF(data);
  ...
  const renderStepContent = (step) => {
    switch (step) {
      case 0:
        return <PostopTKAComplaintForm />;
      case 1:
        return <MedicalHistoryPrompt />;
      case 2:
        return <PostopTKAReviewForm />;
      default:
        return 'Unknown step';
    }
  };
      ...
      <Box sx={{ width: '100%' }}>
            ...
            {activeStep === steps.length - 1 ? (
              <Button variant="contained" onClick={async () => {
                ...
                try {
                  ...
                  const reportText = generatePatientReportText(formData, 'postop');
                  ...
                  // Also generate PDF
                  generatePostopTKAReportPDF(formData);
                  console.log('PDF generation completed');
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            Post-op TKA intake form submitted successfully! PDF report generated.
          </Alert>
        </Snackbar>
      </Box>
    </FormProvider>
