...
import OneYearTHAComplaintForm from './OneYearTHAComplaintForm';
import MedicalHistoryPrompt from './MedicalHistoryPrompt';
import MedicalHistoryForm from './MedicalHistoryForm';
import OneYearTHAReviewForm from './OneYearTHAReviewForm';
import { generateOneYearTHAReportPDF } from '../utils/oneYearThaPdfGenerator';
...
const schema = yup.object().shape({
  ...
  fullRecovery: yup.string().required('Full recovery status is required'),
...
export default function OneYearTHAIntakeForm() {
  ...
  const getFieldsForStep = (step) => {
    switch (step) {
      case 0:
        return [
          'hipSide', 'surgeryType', 'surgeryDate', 'surgeryLocation', 'surgeon',
          'historyChanges', 'fullRecovery', 'normalActivity', 'symptomRelief',
          'ambulationStatus', 'assistiveDevices', 'painMedication', 'satisfaction',
  ...
  const onSubmit = (data) => {
    ...
    try {
      ...
      generateOneYearTHAReportPDF(data);
  ...
  const renderStepContent = (step) => {
    switch (step) {
      case 0:
        return <OneYearTHAComplaintForm />;
      case 1:
        return <MedicalHistoryPrompt />;
      case 2:
        return <OneYearTHAReviewForm />;
      default:
        return 'Unknown step';
    }
  };
      ...
      <Box sx={{ width: '100%' }}>
            ...
            {activeStep === steps.length - 1 ? (
              <Button variant="contained" onClick={async () => {
                ...
                try {
                  ...
                  // Also generate PDF
                  generateOneYearTHAReportPDF(formData);
                  console.log('PDF generation completed');
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            1-year THA intake form submitted successfully! PDF report generated.
          </Alert>
        </Snackbar>
      </Box>
    </FormProvider>
