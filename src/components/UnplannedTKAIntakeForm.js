...
import UnplannedTKAComplaintForm from './UnplannedTKAComplaintForm';
import MedicalHistoryPrompt from './MedicalHistoryPrompt';
import MedicalHistoryForm from './MedicalHistoryForm';
import UnplannedTKAReviewForm from './UnplannedTKAReviewForm';
import { generateUnplannedTKAReportPDF } from '../utils/unplannedTkaPdfGenerator';
...
export default function UnplannedTKAIntakeForm() {
  ...
  const getFieldsForStep = (step) => {
    switch (step) {
      case 0:
        return [
          'kneeSide', 'surgeryType', 'surgeryDate', 'surgeryLocation', 'returnReason',
  ...
  const onSubmit = (data) => {
    ...
    try {
      ...
      generateUnplannedTKAReportPDF(data);
  ...
  const renderStepContent = (step) => {
    switch (step) {
      case 0:
        return <UnplannedTKAComplaintForm />;
      case 1:
        return <MedicalHistoryPrompt />;
      case 2:
        return <UnplannedTKAReviewForm />;
      default:
        return 'Unknown step';
    }
  };
      ...
      <Box sx={{ width: '100%' }}>
            ...
            {activeStep === steps.length - 1 ? (
              <Button variant="contained" onClick={async () => {
                ...
                try {
                  ...
                  const reportText = generatePatientReportText(formData, 'unplanned');
                  ...
                  // Also generate PDF
                  generateUnplannedTKAReportPDF(formData);
                  console.log('PDF generation completed');
        ...
        >
          <Alert onClose={() => setShowSuccess(false)} severity="success">
            Unplanned TKA intake form submitted successfully! PDF report generated.
          </Alert>
        </Snackbar>
      </Box>
    </FormProvider>
