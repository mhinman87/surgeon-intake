...
export const generateOneYearTHAReportPDF = (formData) => {
  console.log('1-year THA PDF Generator called with data:', formData);
  const doc = new jsPDF();
  
  // Set up the document
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('1-year THA / routine long term recheck THA - Patient Report', 20, 30);
  ...
  try {
    ...
    if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
      ...
      link.href = pdfUrl;
      link.target = '_blank';
      link.download = 'one-year-tha-report.pdf';
