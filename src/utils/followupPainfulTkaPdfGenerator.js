...
export const generateFollowupPainfulTKAReportPDF = (formData) => {
  console.log('Follow-up painful TKA PDF Generator called with data:', formData);
  const doc = new jsPDF();
  
  // Set up the document
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Follow-up painful TKA - Patient Report', 20, 30);
  ...
  try {
    ...
    if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
      ...
      link.href = pdfUrl;
      link.target = '_blank';
      link.download = 'followup-painful-tka-report.pdf';
