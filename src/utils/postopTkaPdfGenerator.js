...
export const generatePostopTKAReportPDF = (formData) => {
  console.log('Post-op TKA PDF Generator called with data:', formData);
  const doc = new jsPDF();
  
  // Set up the document
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Post-op TKA - Patient Report', 20, 30);
  ...
  const fields = [
    ...
    { label: 'Knee ROM:', value: `${formData.kneeExtension}° extension, ${formData.kneeFlexion}° flexion` },
  ];
  ...
  try {
    ...
    if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
      ...
      link.href = pdfUrl;
      link.target = '_blank';
      link.download = 'postop-tka-report.pdf';
