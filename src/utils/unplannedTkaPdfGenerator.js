...
export const generateUnplannedTKAReportPDF = (formData) => {
  console.log('Unplanned TKA PDF Generator called with data:', formData);
  const doc = new jsPDF();
  
  // Set up the document
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Unplanned <1-year post-op TKA - Patient Report', 20, 30);
  ...
  try {
    ...
    if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
      ...
      link.href = pdfUrl;
      link.target = '_blank';
      link.download = 'unplanned-tka-report.pdf';
