...
export const generatePreOpTKAReportPDF = (formData) => {
  console.log('Pre-Op TKA PDF Generator called with data:', formData);
  const doc = new jsPDF();
  
  // Set up the document
  doc.setFontSize(16);
  doc.setFont('helvetica', 'bold');
  doc.text('Pre-Op TKA - Patient Report', 20, 30);
  ...
  const fullReportText = generatePatientReportText(formData, 'preop-tka');
  ...
  const fields = [
    { label: 'Knee Side:', value: formData.kneeSide ? formData.kneeSide.charAt(0).toUpperCase() + formData.kneeSide.slice(1) : 'Not specified' },
    { label: 'Diagnosis:', value: formData.diagnosis || 'Not specified' },
    { label: 'History Changes:', value: formData.historyChanges || 'Not specified' },
    { label: 'Conservative Treatment:', value: formData.conservativeTreatment || 'Not specified' },
    { label: 'Treatment Duration:', value: formData.treatmentDuration ? formData.treatmentDuration.charAt(0).toUpperCase() + formData.treatmentDuration.slice(1) : 'Not specified' },
    { label: 'Symptom Impact:', value: formData.symptomImpact || 'Not specified' },
  ...
  // Save the PDF
  const fileName = `PreOp_TKA_Report_${new Date().toISOString().split('T')[0]}.pdf`;
  doc.save(fileName);
  
  console.log('Pre-Op TKA PDF generated successfully:', fileName);
};
